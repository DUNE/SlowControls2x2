...
class ttiPsu(object):
    ...
    #        s.close()
    def send(self, cmd, retries=3, backoff=0.2):
        payload = (cmd + "\n").encode("ascii")
        last_err = None

        for attempt in range(retries):
            try:
                with socket.create_connection((self.ip, self.port), timeout=self.sock_timeout_secs) as s:
                    s.sendall(payload)
                return  # success; context manager closes cleanly
    ...
    def send_receive_string(self, cmd, retries=3, backoff=0.2):
        payload = (cmd + "\n").encode("ascii")
        last_err = None
        ...
        for attempt in range(retries):
            try:
                with socket.create_connection((self.ip, self.port), timeout=self.sock_timeout_secs) as s:
                    s.sendall(payload)
                    data = self.recv_end(s)
                return  data.decode('ascii') # success; context manager closes cleanly
            except (socket.timeout, ConnectionError, OSError) as e:
                last_err = e
                if attempt < retries - 1:
                    time.sleep(backoff)
    ...
    def setTripAmps(self, amps):
        cmd = 'OCP{0} {1:1.4f}'.format(self.channel, amps)
        self.send(cmd)
    ...
    def setMaxAmps(self, amps):
        cmd = 'I{0} {1:1.4f}'.format(self.channel, amps)
        self.send(cmd)
